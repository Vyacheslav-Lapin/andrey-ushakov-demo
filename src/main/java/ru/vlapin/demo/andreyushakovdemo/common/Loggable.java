...
import static ru.vlapin.demo.andreyushakovdemo.common.Loggable.LogLevel.DEBUG;
...
//  @Around("met()")
  ...
  Object aroundAdvice(ProceedingJoinPoint pjp) {
    ...
    logMethod.accept("Метод {} вернул {} для аргументов {}", new Object[]{methodName, result, args});

    return result;
  }

  private BiConsumer<String, Object[]> getLogMethod(ProceedingJoinPoint pjp) {
//    return switch (getAnnotation(pjp, Loggable.class).value()) {
